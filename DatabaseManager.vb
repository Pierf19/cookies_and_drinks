...
Public Class DatabaseManager
    ...
    Public Shared Function LoadCookiesData() As DataTable
        ...
        Try
            Using connection As New MySqlConnection(connectionString)
                ...
                Dim query As String = "SELECT id, name, type, description, image_path FROM cookies ORDER BY name"
                Using command As New MySqlCommand(query, connection)
                    Using adapter As New MySqlDataAdapter(command)
                        adapter.Fill(table)
                    End Using
        ...
        Catch ex As Exception
            MessageBox.Show("Error loading cookies data: " & ex.Message, "Database Error", 
                          MessageBoxButtons.OK, MessageBoxIcon.Error)
    ...
    Public Shared Function LoadDrinkCookiePairingData() As DataTable
        ...
        Try
            Using connection As New MySqlConnection(connectionString)
                ...
                Dim query As String = "SELECT id, drink_id, cookie_id, match_score, pairing_reason FROM drink_cookie_pairing ORDER BY match_score DESC"
                Using command As New MySqlCommand(query, connection)
                    Using adapter As New MySqlDataAdapter(command)
                        adapter.Fill(table)
                    End Using
        ...
        Catch ex As Exception
            MessageBox.Show("Error loading pairing data: " & ex.Message, "Database Error", 
                          MessageBoxButtons.OK, MessageBoxIcon.Error)
    ...
    Public Shared Function GetCookieRecommendationsByDrinkId(drinkId As Integer) As DataTable
        ...
        Try
            Using connection As New MySqlConnection(connectionString)
                ...
                Dim query As String = "SELECT c.id, c.name, c.type, c.description, c.image_path, p.match_score, p.pairing_reason " &
                                    "FROM cookies c " &
                                    "INNER JOIN drink_cookie_pairing p ON c.id = p.cookie_id " &
                                    "WHERE p.drink_id = @drinkId " &
        ...
        Catch ex As Exception
            MessageBox.Show("Error loading cookie recommendations: " & ex.Message, "Database Error", 
                          MessageBoxButtons.OK, MessageBoxIcon.Error)
    ...
    ' Method untuk menambah minuman baru (admin features)
    ...
    ' Method untuk menambah cookie baru (admin features)
    ...
    ' Method untuk menambah pairing baru (admin features)
